        ...
        if key == ModeKeys.TRAIN:
            ...
            def mwoz_eval_callback(scaffold, loader):
                ...
                for j, data_dev in enumerate(loader):
                    ...
                    if args["analyze"]:
                        _, gates, words, class_words, analyze, rank = model.encode_and_decode(
                            data_dev, False)
                    else:
                        _, gates, words, class_words, rank = model.encode_and_decode(
                            data_dev, False)

                    rank_idx = []
                    for islot, slot in enumerate(slot_temp):
                        rank_idx.append(rank[islot].argmax(1))
                    rank_idx = torch.stack(rank_idx, 1)
                    ...
                    for bi in range(batch_size):
                        ...
                        if args["use_gate"]:
                            for si, sg in enumerate(gate):
                                analyze[bi]['infer'][slot_temp[si]] = {
                                    "gate": gating_dict.token_of(sg.tolist())}
                                tpks, tpki = torch.topk(rank[si][bi], min(5, rank[si].size(1)))
                                tpkv = model.ont_vocabs[slot_temp[si]].index2token(tpki.tolist())
                                analyze[bi]['infer'][slot_temp[si]]['rank'] = list(zip(tpkv, tpks.tolist()))

                                if sg == gating_dict.index_of("none"):
                                    continue
                                elif sg == gating_dict.index_of("ptr"):
                                    ont_token = model.ont_vocabs[slot_temp[si]].token_of(rank_idx[bi][si].item())
                                    if ont_token == 'none':
                                        ...
                                        for e in pred:
                                            if e == EOS_TOKEN:
                                                ...
                                                st.append(e)
                                        st = " ".join(st)
                                        analyze[bi]['infer'][slot_temp[si]]['gen'] = st
                                        if st == "none":
                                            continue
                                        else:
