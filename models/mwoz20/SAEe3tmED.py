...
from clks.nnet.rnn import LSTMCell, GRUCell
...
class TopicModel(nn.Module):
    ...
    def forward(self, input_tensor, input_lengths, segments):
        ...
        for ib, (ten, seg) in enumerate(zip(input_tensor, segments)):
            ...
            esc = esc.masked_fill(sub_msk == 0, -1e9)
    ...
    def compute_loss(self, yzpr_list):
...
class Generator(nn.Module):
    def __init__(self, args, config, word_vocab, shared_emb, 
        ...
        self.gru = GRUCell(hidden_size, hidden_size, drop_method='output', drop_prob=dropout)
            ...
            for islot, slot in enumerate(slot_temp):
                ...
                for wi in range(max_res_len):
                    if wi == 0:
                        ...
                        dec_state, hidden = self.gru(decoder_input.expand_as(hidden), state=hidden)
...
class SAEe3tmED(Model):
    ...
    def compute_loss(self, data):
        ...
        (all_point_outputs, gates, words_point_out, 
            words_class_out, kld_reg_loss, zpr_list) = self.encode_and_decode(
        ...
        L_tm, tmdisp = self.tm.compute_loss(zpr_list)
