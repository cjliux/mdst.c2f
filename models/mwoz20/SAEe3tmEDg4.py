...
class Generator(nn.Module):
    def __init__(self, args, config, word_vocab, shared_emb, 
        ...
        # edg v2: topic gate
        self.W_gate_ctx = nn.Linear(hidden_size, nb_gate)
        self.W_gate_topic = nn.Linear(args["num_topic"], nb_gate)
        self.W_gate_slot = nn.Linear(hidden_size, nb_gate)
    ...
    def forward(self, batch_size, encoded_hidden, encoded_outputs, encoded_lens, 
        ...
        if True:
            ...
            for islot, slot in enumerate(slot_temp):
                ...
                prior_tp = torch.matmul(prior_alpha.clone().detach().unsqueeze(1), topic_repr).squeeze(1)
                ...
                if self.training:
                    ...
                    postr_tp = torch.matmul(postr_alpha.clone().detach().unsqueeze(1), topic_repr).squeeze(1)
                ...
                if self.training:
                    ctx, topic = postr_ctx, postr_tp
                else:
                    ctx, topic = prior_ctx, prior_tp
                init_state = slot_ctrl.expand_as(hidden)
                hidden = ctx.expand_as(hidden)
                all_gate_outputs[islot] = self.W_gate_ctx(ctx) + self.W_gate_topic(topic) + self.W_gate_slot(slot_ctrl)
                ...
                for wi in range(max_res_len):
                    if wi == 0:
                        ...
                        dec_state, hidden = self.gru(hidden, topic, state=init_state)
                    else:
