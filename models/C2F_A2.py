        ...
        for islot, slot in enumerate(slot_temp):
            ...
            for wi in range(max_res_len):
                ...
                if self.args["analyze"] and wi == 0:
                    ...
                    tgt_w = [' '.join(self.mem_vocab.index2token([w.item() for w in inst[islot] if w not in [1, 2]])) for inst in target_batches]
...
class C2F_A2(Model):
    ...
    def compute_loss(self, data):
        ...
        if self.args["use_gate"]:
            loss = loss_ptr + loss_gate + loss_rank
        else:
            loss = loss_ptr + loss_rank

        return loss, { "LP": loss_ptr, "LG": loss_gate, "LR": loss_rank }
