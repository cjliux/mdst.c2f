...
class Generator(nn.Module):
    def __init__(self, args, config, word_vocab, mem_vocab, shared_emb, 
    ...
    def forward(self, batch_size, encoded_hidden, encoded_outputs, encoded_lens, 
        ...
        if self.args["analyze"]:
            return all_point_outputs, all_gate_outputs, words_point_out, [], analyze
        else:
...
class AutoBase(Model):
    ...
    def __init__(self, args, config):
        ...
        self.encoder = EncoderRNN(args, config, self.word_vocab, self.mem_vocab,
            self.hidden_size, self.dropout)
        self.decoder = Generator(args, config, 
            self.word_vocab, self.mem_vocab, self.encoder.embedding, 
            self.hidden_size, self.dropout, 
    ...
    def encode_and_decode(self, data, use_teacher_forcing, slot_temp=None):
        ...
        if self.training:
            ...
            if self.args["analyze"]:
                all_point_outputs, all_gate_outputs, words_point_out, words_class_out, analyze = self.decoder.forward(batch_size, 
                    encoded_hidden, encoded_outputs, data['context_len'], 
                    data["context"], story, ptr_story, max_res_len, data['generate_y'], 
                    use_teacher_forcing, slot_temp) 
                return all_point_outputs, all_gate_outputs, words_point_out, words_class_out, analyze
            else:
    ...
    def compute_loss(self, data):
        ...
        (all_point_outputs, gates, words_point_out, 
            words_class_out) = self.encode_and_decode(
