...
fin = open(os.path.join(file_dir, 'mapping.pair'),'r')
replacements = []
...
#         resp = open(os.path.join(args["save_path"], "MULTIWOZ2.1.zip"))
#         zip_ref = ZipFile(BytesIO(resp.read()))
#         zip_ref.extractall(args["save_path"])
#         zip_ref.close()
#         shutil.move(
#             os.path.join(args["save_path"], "MULTIWOZ2 2"), 
#             os.path.join(args["save_path"], "mwoz21")
#         )
...
def createData():
    ...
    fin1 = open(os.path.join(args["save_path"], "mwoz21", 'data.json'), 'r')
    data = json.load(fin1)

    fin2 = open(os.path.join(args["save_path"], "mwoz21", 'dialogue_acts.json'), 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open(os.path.join(args["save_path"], "mwoz21", 'testListFile.json'), 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open(os.path.join(args["save_path"], "mwoz21", 'valListFile.json'), 'r')
    for line in fin:
        valListFile.append(line[:-1])
    fin.close()

    proc_dir = os.path.join(args["save_path"], "mwoz21", 'proc')
    if not os.path.exists(proc_dir):
        os.makedirs(proc_dir)
    trainListFile = open(os.path.join(proc_dir, 'trainListFile'),
        'w', encoding='utf8')
    ...
    # save all dialogues
    with open(os.path.join(args["save_path"], "mwoz21", 'proc', 'dev_dials.json'), 'w') as f:
        json.dump(val_dials, f, indent=2)

    with open(os.path.join(args["save_path"], "mwoz21", 'proc', 'test_dials.json'), 'w') as f:
        json.dump(test_dials, f, indent=2)

    with open(os.path.join(args["save_path"], "mwoz21", 'proc', 'train_dials.json'), 'w') as f:
        json.dump(train_dials, f, indent=2)
