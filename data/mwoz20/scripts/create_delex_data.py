...
def loadData():
    ...
    if not os.path.exists(data_url):
        ...
        shutil.move(
            ...
            os.path.join(args["save_path"], "mwoz20")
        )
...
def createData():
    ...
    fin1 = open(os.path.join(args["save_path"], "mwoz20", 'data.json'), 'r')
    data = json.load(fin1)

    fin2 = open(os.path.join(args["save_path"], "mwoz20", 'dialogue_acts.json'), 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open(os.path.join(args["save_path"], "mwoz20", 'testListFile.json'), 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open(os.path.join(args["save_path"], "mwoz20", 'valListFile.json'), 'r')
    for line in fin:
    ...
    # save all dialogues
    with open(os.path.join(args["save_path"], "mwoz20", 'proc', 'dev_dials.json'), 'w') as f:
        json.dump(val_dials, f, indent=4)

    with open(os.path.join(args["save_path"], "mwoz20", 'proc', 'test_dials.json'), 'w') as f:
        json.dump(test_dials, f, indent=4)

    with open(os.path.join(args["save_path"], "mwoz20", 'proc', 'train_dials.json'), 'w') as f:
