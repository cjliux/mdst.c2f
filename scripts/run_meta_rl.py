...
try:
    ...
    from .config_meta_rl import args
except:
    from __conf__ import *
    from config_meta_rl import args
...
import utils.meta_rl as meta_rl
ModeKeys = meta_rl.ModeKeys
...
def train(args):
    ...
    if args["dataset"] == "mwoz20":
        from data.mwoz20.interface_meta_rl import prepare_data_loader_4_meta_rl
        (train_loader, dev_loader, test_loader,
            test_4d_loader) = prepare_data_loader_4_meta_rl(
                args, args["data_path"], training=True)
        from models.mwoz20.interface_meta_rl import build_model_fn_4_meta_rl
        ...
        scf = meta_rl.MetaRLLearner().build(
            args, config, os.path.join(ENV_PATH, "meta_rl_trained"), args["run_id"])
...
def evaluate(args):
    ...
    if args["dataset"] == "mwoz20":
        ...
        if args["run_dev_testing"]:
            ...
            eval_spec = model_fn(ModeKeys.EVAL, dev_loader)
            scf = meta_rl.MetaRLLearner().build(args, config,
                        os.path.join(ENV_PATH, "meta_rl_trained"), args["run_id"])
